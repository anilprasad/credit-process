...
export default async (
  ...
  const sagemaker_xgb = mlmodel[ 'sagemaker_xgb' ];
  ...
  const providerDatasource = datasource.providers.sagemaker_xgb;
  ...
  const params = {
    ...
    EndpointName: sagemaker_xgb.real_time_prediction_id,
  ...
  const responseBody = JSON.parse(Buffer.from(response.Body as string).toString('utf8')) as number[] | number ;
  let score: number | null = null;
  let predicted_label: number | null = null;
  let predicted_label_idx: number | null = null;
  if (modelType === 'binary' || modelType === 'regression') {
    score = Number(responseBody);
  } else if (modelType === 'categorical') {
    score = Math.max(...responseBody as number[]);
    predicted_label_idx = (responseBody as number[]).indexOf(score);
  ...
  await Promise.all(datasourceHeaders.map(async (header, i) => {
    ...
    if (modelType === 'binary' || modelType === 'regression') {
      explainabilityResult = Number(explainabilityResultBody);
    } else if (modelType === 'categorical') {
      explainabilityResult = (explainabilityResultBody as number[])[predicted_label_idx!];
    }
