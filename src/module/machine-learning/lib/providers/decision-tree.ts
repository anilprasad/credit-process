...
export default async (
  ...
  const decisionTree = mlmodel['decision_tree'];
  ...
  const model_config = JSON.parse(decisionTree.model);
  const classifier = (modelType === 'regression')
    ? DecisionTreeRegression.load(model_config)
    : DecisionTreeClassifier.load(model_config);

  const predictions = (modelType === 'binary')
    ? helpers.runDecisionTreePrediction(classifier, cleaned)
    : (modelType === 'categorical')
      ? helpers.runDecisionTreePredictionCategorical(classifier, cleaned)
      : classifier.predict(cleaned);

  const predictionRow = predictions[0];
  ...
  datasourceHeaders.forEach((header, i) => {
    ...
    const explainabilityResultArray = (modelType === 'binary')
      ? helpers.runDecisionTreePrediction(classifier, [newCleaned])
      : (modelType === 'categorical')
        ? helpers.runDecisionTreePredictionCategorical(classifier, [newCleaned])
        : classifier.predict([newCleaned]);
