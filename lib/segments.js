 ...
 */
var createSegmentEvaluator = function (segments, skip_evaluator, type, module_name = null) {
  return function segment_evaluator(state, return_evaluator = false) {
    try {
      ...
      if (skippable && (!valid || (Array.isArray(valid) && !valid.length) || !skippable.strict_skip)) {
        return _state;
      }

      if (valid && return_evaluator === true) {
        if (Array.isArray(valid)) {
          return valid.map(segment => segment.evaluator);
        } else {
          return valid.evaluator;
        }
      }

      let evaluations = (Array.isArray(valid)) ? valid.reduce((result, segment) => {
        if (segment && typeof segment.evaluator !== 'function') throw new Error(segment.message);
      ...
      return Promisie.reject(Object.assign({}, state, { message: `Error while creating segment evaluator - ${type} module ${module_name || ''}: ${e.message}`, }));
    }
  };
};
 ...
 */
var compileSegments = function (type, module_name = null, configurations) {
  ...
  return createSegmentEvaluator(segments, skip_evaluator, type, module_name);
};
