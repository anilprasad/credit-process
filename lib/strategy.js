...
var createCreditPipeline = function (operations) {
  try {
    return Object.keys(operations).reduce((pipeline, segmentKey) => {
      if (segmentKey) {
        pipeline = pipeline.concat(operations[segmentKey]);
      }
...
var compileCreditEngine = function (options = {}) {
  ...
  return Promisie.parallel(operations)
    .then(result => createCreditPipeline(result))
    .catch(error => Promisie.reject(error));
};
